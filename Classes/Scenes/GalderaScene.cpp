...
{
    FiniteTimeAction *actionMove = MoveBy::create(0.2, Vec2(VisibleRect::getVisibleRect().size.width, 0));
...
void GalderaScene::changeGaldera(Node* sender)
{
    ...
    if (currentGalderaPosition == (galderaCount + 1)) {
        ...
        galderaLayer->setPosition(Vec2(-visualRect.size.width, 0));
        FiniteTimeAction *actionMove = MoveBy::create(0.2, Vec2(visualRect.size.width, 0));
