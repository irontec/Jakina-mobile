...
bool LevelScene::init()
{
    ...
    for (int i = 1; i <= numPages; i++) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            pMultzoaTitle->setPosition(Vec2(pAreaLayer->getContentSize().width/2 , 60));
...
SpriteButton* LevelScene::getButton(MultzoaModel *multzoa)
{
    ...
    auto pButton = SpriteButton::create(ImageManager::getImage(name->c_str()), 0.8f, CC_CALLBACK_1(LevelScene::levelSelected, this));
    delete name;
    return pButton;
}
