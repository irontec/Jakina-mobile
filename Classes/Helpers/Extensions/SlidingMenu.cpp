...
Point SlidingMenuGrid::GetPositionOfCurrentPage()
{
	...
	Point position=getPosition();
	if(bVerticalPaging)
		position=Vec2(0,iCurrentPage*winSize.height);
	else
		position=Vec2(-(iCurrentPage)*winSize.width,0);
	return position;
}
...
void SlidingMenuGrid::setPageLabel( int iPage,Node * pLabel)
{
	...
	if(bVerticalPaging)
		pt=Vec2(menuOrigin.x+labelSize.width/2,winSize.height-menuOrigin.y-winSize.height*(iSetPage-1)-iMenuRows*padding.y-labelSize.height/2);
	else
		pt=Vec2(menuOrigin.x+(iSetPage-1)*winSize.width+labelSize.width/2,winSize.height-menuOrigin.y-iMenuRows*padding.y-labelSize.height/2);
