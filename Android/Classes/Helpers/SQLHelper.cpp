...
void SQLHelper::insertKategoria(CCArray *kategoriak)
{
    clock_t cStartClock;
    cStartClock = clock();
    sqlite3_stmt *stmt;
    ...
    for (int i = 0; i < kategoriak->count(); i++) {
        pKategoria = (KategoriaModel*) kategoriak->objectAtIndex(i);
        ...
        if (result != SQLITE_DONE) {
            CCLOG("ERROR SQLITE %d INSERTING ROW: idGaldera: %i, Galdera %s ", result, pKategoria->idKategoria, pKategoria->izena.c_str());
        }
        sqlite3_clear_bindings(stmt);
        sqlite3_reset(stmt);
    }
    sqlite3_exec(_db, "END TRANSACTION", NULL, NULL, &sErrMsg);
    printf("KATEGORIA - Imported %d records in %4.2f seconds\n",kategoriak->count(), (clock() - cStartClock) / (double)CLOCKS_PER_SEC);
    sqlite3_finalize(stmt);

}

void SQLHelper::insertGaldera(CCArray *galderak)
{
    clock_t cStartClock;
    cStartClock = clock();
    sqlite3_stmt *stmt;
    ...
    for (int i = 0; i < galderak->count(); i++) {
        pGaldera = (GalderaModel*) galderak->objectAtIndex(i);
        ...
        int result = sqlite3_step(stmt);
        if (result != SQLITE_DONE) {
            CCLOG("ERROR SQLITE %d INSERTING ROW: idGaldera: %i, Galdera %s ", result, pGaldera->idGaldera, pGaldera->galdera.c_str());
        }
        sqlite3_clear_bindings(stmt);
        sqlite3_reset(stmt);
    }
    sqlite3_exec(_db, "END TRANSACTION", NULL, NULL, &sErrMsg);
    printf("GALDERAK - Imported %d records in %4.2f seconds\n",galderak->count(), (clock() - cStartClock) / (double)CLOCKS_PER_SEC);
    sqlite3_finalize(stmt);
}

void SQLHelper::insertMultzoa(CCArray *multzoak)
{
    clock_t cStartClock;
    cStartClock = clock();
    sqlite3_stmt *stmt;
    ...
    for (int i = 0; i < multzoak->count(); i++) {
        pMultzoa = (MultzoaModel*) multzoak->objectAtIndex(i);
        ...
        int result = sqlite3_step(stmt);
        if (result != SQLITE_DONE) {
            CCLOG("ERROR SQLITE %d INSERTING ROW: idGaldera: %i, Galdera %s ", result, pMultzoa->idMultzoa, pMultzoa->izena.c_str());
        }
        sqlite3_clear_bindings(stmt);
        sqlite3_reset(stmt);
    }
    sqlite3_exec(_db, "END TRANSACTION", NULL, NULL, &sErrMsg);
    printf("GALDERAK - Imported %d records in %4.2f seconds\n", multzoak->count(), (clock() - cStartClock) / (double)CLOCKS_PER_SEC);
    sqlite3_finalize(stmt);
}
