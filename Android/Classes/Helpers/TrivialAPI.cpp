...
void TrivialAPI::parseKategoriak(std::string str)
{
    ...
    if (kategoriak != NULL) {
        ...
        CCArray *kategoriakModels = CCArray::create();
        for (int i = 0; i < numKategoriak; i++)
        {
            ...
            kategoriakModels->addObject(katModel);
        }
